...
static void esp_gap_cb(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param)
{
    ...
    esp_err_t err;
       const uint8_t mac[6] = {0x24, 0x0a, 0xc4, 0x85, 0xcd, 0x96};
            uint8_t mac_is_same = 1;
    ...
    case ESP_GAP_BLE_SCAN_RESULT_EVT: {
        ...
        case ESP_GAP_SEARCH_INQ_RES_EVT:
            ...
            for (int i = 0; i < 6; i++) {
                if (mac[i] != scan_result->scan_rst.bda[i]) {
                    mac_is_same = 0;
                    break;
                }
            } 
            ...
            if (mac_is_same) {
                ESP_LOGW(GATTC_TAG, "device is found");
                esp_log_buffer_hex(GATTC_TAG, scan_result->scan_rst.bda, 6);
                ESP_LOGW(GATTC_TAG, "rssi = %d", scan_result->scan_rst.rssi);
