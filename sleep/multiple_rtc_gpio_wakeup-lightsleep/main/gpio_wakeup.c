...
{
    printf("Waiting for GPIO7 to go low...\n");
    while (gpio_get_level(GPIO_NUM_7) == 1) {
        vTaskDelay(pdMS_TO_TICKS(10));
    }

    printf("Waiting for GPIO10 to go low...\n");
    while (gpio_get_level(GPIO_NUM_10) == 1) {
        vTaskDelay(pdMS_TO_TICKS(10));
    }

        printf("Waiting for GPIO20 to go low...\n");
    while (gpio_get_level(GPIO_NUM_20) == 1) {
        vTaskDelay(pdMS_TO_TICKS(10));
    }
...
esp_err_t example_register_gpio_wakeup(void)
{
    ...
    gpio_config_t config = {
            .pin_bit_mask = (BIT64(GPIO_NUM_7) | BIT64(GPIO_NUM_10) | BIT64(GPIO_NUM_20)),
    ...
    ESP_RETURN_ON_ERROR(gpio_wakeup_enable(GPIO_NUM_7, GPIO_INTR_HIGH_LEVEL),
                        TAG, "Enable gpio 10 wakeup failed");
    ESP_RETURN_ON_ERROR(gpio_wakeup_enable(GPIO_NUM_10, GPIO_INTR_HIGH_LEVEL),
                        TAG, "Enable gpio 10 wakeup failed");
    ESP_RETURN_ON_ERROR(gpio_wakeup_enable(GPIO_NUM_20, GPIO_INTR_HIGH_LEVEL),
                        TAG, "Enable gpio 20 wakeup failed");
