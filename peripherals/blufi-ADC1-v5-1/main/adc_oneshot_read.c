...
void adc_task(void)
{
    ...
    while (1) {
        ...
        vTaskDelay(pdMS_TO_TICKS(1000));
        printf("ADC1 Channel 2\n");

        ESP_ERROR_CHECK(adc_oneshot_read(adc1_handle, EXAMPLE_ADC1_CHAN2, &adc_raw[0][2]));
        ESP_LOGI(TAG, "ADC%d Channel[%d] Raw Data: %d", ADC_UNIT_1 + 1, EXAMPLE_ADC1_CHAN2, adc_raw[0][2]);
        if (do_calibration1_chan1) {
        ESP_ERROR_CHECK(adc_cali_raw_to_voltage(adc1_cali_chan2_handle, adc_raw[0][2], &voltage[0][2]));
        ESP_LOGI(TAG, "ADC%d Channel[%d] Cali Voltage: %d mV", ADC_UNIT_1 + 1, EXAMPLE_ADC1_CHAN2, voltage[0][2]);
        }
        vTaskDelay(pdMS_TO_TICKS(1000));
